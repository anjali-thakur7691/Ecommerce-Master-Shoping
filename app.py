...
def profileHome():
    if 'email' not in session:
        return redirect(url_for('root'))
    loggedIn, firstName, noOfItems = getLoginDetails()
    return render_template("profileindex.html", loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems)
...
def parse(data):
    # यह फंक्शन डेटा को 3-3 के ग्रुप में बांटता है ताकि ग्रिड सही दिखे
    ans = []
    for i in range(0, len(data), 3):
        ans.append(data[i:i+3])
    return ans
